 ...
 * possible:
 * 
 * 1.) Fork https://github.com/overlordtester/camel-github-test, then create a
 * new pull request with a WSDL update. 2.) DTGov validates the WSDL and
 * discovers an issue. (For this specific demo, the validation will be fake.)
 * 3.) DTGov comments on the pull request, describing why it was invalidated.
 * 4.) DTGov automatically rejects the pull request. 5.) DTGov posts a Tweet,
 * describing the failure, to a central account. (Theoretically, this could also
 * be configured to Tweet *at* a user, send a direct message, etc.) 6.) The
 * developer corrects the WSDL and submits a new pull request. 7.) DTGov
 * successfully validates the WSDL. 8.) DTGov comments on the pull request,
 * mentioning that it's valid.
 * 
 * To run the demo:
 * 
 * 1.) mvn exec:java
 * -Dexec.mainClass=org.overlord.demo.dtgov.camel.WSDLValidation
 * -Dexec.args='-t' (The -t option will print out Camel Route tracing -- useful
 * for demos.) 2.) Fork https://github.com/overlordtester/camel-github-test,
 * commit *anything*, and create a pull request. IMPORTANT: Title the pull
 * request "invalid". This will tell the fake validator to take the error path.
 * The commit itself is irrelevant. 3.) Verify that DTGov commented on the pull
 * request, closed it, and Tweeted about it. 4.) Repeat #3, but title it
 * anything *other than* "invalid". 5.) Verify that DTGov commented on the pull
 * request.
 * 
 * GitHub: username "overlordtester", password "overlord1!" Twitter: username
 * "overlordtester", password "overlord1!", keys created through
 * https://apps.twitter.com/app/6988265
 * 
 * @author Brett Meyer
 */
public class WSDLValidation extends Main {
    ...
    public static class WSDLValidationRouteBuilder extends RouteBuilder {
        ...
        public void configure() throws Exception {
            ...
            // TODO: Also needs to close the PR, so may need to wiretap or
            // in/out in here. In/out *before* the Tweet
            // might be best, but make sure the new ClosePR endpoint adds the
            // PullRequest object back to the Exchange.
            from("direct:invalidPullRequest")
                    .process(new InvalidPullRequestProcessor())
                    .multicast().parallelProcessing()
                            .to("github://pullRequestComment?username=" + GITHUB_USERNAME + "&password="
                                    + GITHUB_PASSWORD + "&repoOwner=" + GITHUB_REPO_OWNER + "&repoName="
                                    + GITHUB_REPO_NAME)
                            .to("twitter://timeline/user?consumerKey=" + TWITTER_CONSUMER_KEY + "&consumerSecret="
